...
SDtab = data.table::fread(unzip(paste0(path,"dwca-seychecklist-v1.9.zip"), "occurrence.txt"), header = TRUE, sep = "\t", dec = ".", encoding = "UTF-8", quote="")
SDtab <- SDtab %>% dplyr::select(-taxonID) %>% rename(taxonID = id)
SP = data.table::fread(unzip(paste0(path,"dwca-seychecklist-v1.9.zip"), "taxon.txt"), header = TRUE, sep = "\t", dec = ".", encoding = "UTF-8", quote="") %>% dplyr::select(-id)
SPmeta = data.table::fread(unzip(paste0(path,"dwca-seychecklist-v1.9.zip"), "speciesprofile.txt"), header = TRUE, sep = "\t", dec = ".", encoding = "UTF-8", quote="") %>% dplyr::rename(taxonID=id)
SPdistrib = data.table::fread(unzip(paste0(path,"dwca-seychecklist-v1.9.zip"), "distribution.txt"), header = TRUE, sep = "\t", dec = ".", encoding = "UTF-8", quote="") %>% dplyr::rename(taxonID=id)
ID = data.table::fread(unzip(paste0(path,"dwca-seychecklist-v1.9.zip"), "identifier.txt"), header = TRUE, sep = "\t", dec = ".", encoding = "UTF-8", quote="")  %>% dplyr::rename(taxonID=id)
...
```{r eval = FALSE}
Cad1 = sf::st_read("D:/ZAnalysis/ZDraft/datainput/sey_Adm_cadastral_2021dd_ownership.shp", crs = 4326, quiet = TRUE)
Cad0 = sf::st_read("D:/ZAnalysis/ZDraft/datainput/sey_Adm_cadastral_2021dd_NOCADASTRE.shp", crs = 4326, quiet = TRUE)
#Load the metadata table on cadastral parcels
CadData <- read.csv("D:/ZAnalysis/ZDraft/datainput/sey_cad_ownership.csv", header = TRUE, sep = ";", encoding = "Windows-1252")#"UTF-8") #\t

WS = sf::st_read("D:/ZAnalysis/ZDraft/datainput/sey_watershed.shp", crs = 4326, quiet = TRUE)

DEM <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_DEM_Lidar_Jaxa.tif")
LC <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_LC_obia.tif")
EcoGe_cur <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_EcoGe_cur.tif")
EcoGe_pot <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_EcoGe_pot.tif")
EcoGe_pre <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_EcoGe_pre.tif")
LUP <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_LUP.tif")
Nat <- raster::raster("D:/ZAnalysis/ZDraft/datainput/Senterre_2021_SeyInner_Naturalness.tif")
...
```{r}
KBA = sf::st_read("datainput/sey_cons_pas2023.shp", crs = 4326, quiet = TRUE) %>% subset(statusID != 8)
...
KBA$isPA2022 <- ifelse(KBA$kbaName == "Fond Azore-Fond Ferdinand proposed Ecological Reserve" | KBA$kbaName == "Collines Du Sud proposed Ecological Reserve" | KBA$kbaName == "Montagne Planneau proposed National Park" | KBA$kbaName == "Anse Kerland-Midlands proposed Sustainable Use Area","2-Proposed",KBA$isPA2022)

KBAmeta <- dplyr::select(as.data.frame(KBA), -geometry) 
#to convert a sf object to a simple table (data.frame)

KBAmeta <- with(KBAmeta, KBAmeta[order(statusID, islGroup, kbaName) , ])
...
SDtabxxxKBA3 <- SDtabxxxKBA2 %>% dplyr::group_by(occurrenceID) %>% dplyr::summarize(kbaName = first(kbaName), statusID = first(statusID), status = first(status),isPA = first(isPA),isPA2022 = first(isPA2022))
