...
library(shinycssloaders) #For showing a loading icon while loading maps
#library(imager)
...
KBA = sf::st_read("sey_cons_pas2023.shp", crs = 4326, quiet = TRUE) %>% subset(statusID != 8)
...
kbaList <- sort(unique(SDint[nchar(SDint$kbaName)>2,]$kbaName)) %>% as.data.frame() %>% rename(nametxt = 1)
kbaList$scale <- "kba"
    ...
    tabItems(
      ...
      tabItem(tabName = "map",
              fluidRow(
                ...
                column(11.7,
                       leaflet::leafletOutput("mymap") %>% withSpinner()#, width = "100%", height = "100%")#,
                       #imageOutput("myLogo")#img(src='Bio_logo.png', align = "left")
                ),
                #verbatimTextOutput("counter"),
                p("App version bioflora 1.3, based on ", tags$a(href="https://www.gbif.org/dataset/d0b7cc29-8ce3-49ad-b1b6-675fd39412ee", "seychecklist v1.9"))
              )
...
#Then I need to get the SDintAOI as a reactive function's return, to be called like an input
  ...
  SDintAOI_r <- reactive({
    ...
    if(input$scaleSelect == "kba") {SDintAOI <- SDint[SDint$kbaName == input$elementOfFilter,]}
...
#Render the map (= just same as reactive function)
  output$mymap <- renderLeaflet({
    ...
    if(input$scaleSelect == "kba") { # && !is.null(input$elementOfFilter)) {
      KBAselected <- KBA[KBA$kbaName == input$elementOfFilter,]
...
#Get another tab with more species stats, especially relevant for KBAs
  output$mytable_geoaccuracyStat <- renderDT({
    ...
    NatTab2 <- dplyr::left_join(speciesList_r(), SDgeoaccuracyAOI, by = "taxonID") #speciesList_r()=SDintxAOI
    ...
    #Find out if it is a KBA trigger (Global or Regional)
    #Normally, A1 only Global status CR-EN & > 0.5% OR VU & > 1% OR B1 > 10%
    NatTab2DT <- setDT(NatTab2)
    NatTab2DT <- NatTab2DT[, kbaTrigger := ifelse(ratio > 5 & ((stringr::str_detect(establishmentMeansNat, "native") & (threatStatusGlobal == "CR" | threatStatusGlobal == "EN")) | (stringr::str_detect(establishmentMeansNat, "ndemic") & (threatStatusNat == "CR" | threatStatusNat == "EN"))), "Global",
                                                  ifelse(ratio > 10 & stringr::str_detect(establishmentMeansNat, "native") & threatStatusGlobal == "VU", "Global",
                                                         ifelse(ratio > 10 & stringr::str_detect(establishmentMeansNat, "native") & (threatStatusNat == "CR" | threatStatusNat == "EN"), "Regional",NA)
                                                         )
                                                  )]
    NatTab2DT <- NatTab2DT[, iucn := paste0(threatStatusGlobal, "|", threatStatusNat)]

    #Get the table to renderDT
    NatTab2 <- NatTab2DT[,c("species_short", "vernacularName", "toCoord", "toLoc", "vague", "isNA", "nObs", "nObsNat", "ratio", "kbaTrigger", "iucn", "firstYear", "lastYear")]# %>% rename(occStatus=occurrenceStatusNat)
    NatTab2 <- NatTab2[order(NatTab2$kbaTrigger, -NatTab2$ratio, NatTab2$species_short),]
    DT::datatable(NatTab2[, 1:13],
                  extensions='Buttons',
                  options = list(dom='Blfrtip', #Bfrtip is for buttons
                                 buttons =
                                   list(
